...
$('#checkInData').ready(function(){
    $.ajax({
        url:"../checks/checkItem.php",
        method:"POST",
        dataType:"text",
        success:function(data){
            $('#checkInData').html(data);
        },
        error: function (err) {
            alert("Data Parsing Error => " + err);
        }
    });
});
$(document).ready(function(){
    ...
    /*$('table#items-check tbody tr').ready(function() {
        var t = $('data');
        var id = $(this).data('id');
    ...
    function submit_check(from, t) {
        ...
        if(actpage == 'in'){
            var phppage = '../check-in/update.php';}
        else {
            var phppage = '../check-in/update.php';
            //alert("worked!");
        ...
        //$('#error').html("something wrong for test "+prev_value);
        $.post(phppage, {
            'act':'0',
            'id':id,
            'fromval':prev_value,
            'val': parseInt(prev_value)-parseInt(val)
        }, function(data) {
            if(data == '2') {
                alert('You cannot check out more than '+prev_value+' items');
                return false;
